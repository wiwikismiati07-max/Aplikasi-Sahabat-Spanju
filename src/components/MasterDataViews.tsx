...
interface MasterSiswaViewProps {
  ...
  onImportBulkSiswa: (list: SiswaMaster[]) => void;
  ...
  const handleExcelImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (parsed.length > 0) {
          onImportBulkSiswa(parsed);
          alert(`Berhasil mengimpor ${parsed.length} data siswa ke dalam sistem!`);
        }
      } catch (err) {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-100 max-w-xl">
              Pusat data siswa UPT SMPN 7 Pasuruan terhubung otomatis ke seluruh fitur: Piket Harian, SP Damai, E-Lapor, dan Duta Anti-Bullying.
            </p>
          </div>
        </div>
          ...
          {isAdmin && (
            <>
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-2.5 bg-white/15 hover:bg-white/25 text-white rounded-xl text-xs font-semibold transition-colors"
                  ...
                  <td className="py-2.5 px-4">
                    <span
                      className={`font-semibold ${
                        s.jenisKelamin === 'L' ? 'text-blue-600' : 'text-pink-600'
                      }`}
                    >
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col my-auto max-h-[92vh] animate-in fade-in zoom-in-95 duration-150">
            <div className="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between bg-emerald-700 text-white flex-shrink-0">
              <div>
                <h3 className="text-base font-bold">Tambah Siswa Baru</h3>
                <p className="text-xs text-emerald-100 mt-0.5">Input manual satu siswa ke sistem</p>
              </div>
              <button
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 text-xs bg-slate-50 border border-slate-300 rounded-xl focus:ring-2 focus:ring-emerald-500 text-slate-800"
                  >
                    {['7A', '7B', '7C', '7D', '7E', '7F', '7G', '7H',
                      '8A', '8B', '8C', '8D', '8E', '8F', '8G', '8H',
                      '9A', '9B', '9C', '9D', '9E', '9F', '9G', '9H'].map((k) => (
...
interface MasterGuruViewProps {
  ...
  onImportBulkGuru: (list: GuruMaster[]) => void;
  ...
  const handleExcelImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (parsed.length > 0) {
          onImportBulkGuru(parsed);
          alert(`Berhasil mengimpor ${parsed.length} data guru/staf ke dalam sistem!`);
        }
      } catch (err) {
        alert('Gagal membaca file Excel. Pastikan format kolom sesuai dengan template.');
      }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-blue-100 max-w-xl">
              Data resmi guru pengajar, wali kelas 24 rombel, tim konseling BK, dan Satgas TPPK UPT SMPN 7 Pasuruan.
            </p>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {filtered.map((g, idx) => (
                <tr key={g.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-2.5 px-4 text-slate-600">{g.statusKepegawaian || 'PNS'}</td>
                  {isAdmin && (
                    <td className="py-2.5 px-4 text-center">
                      <button
                        type="button"
