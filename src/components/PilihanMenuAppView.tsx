...
export const PilihanMenuAppView: React.FC<PilihanMenuAppViewProps> = ({
  ...
  const allCards: MenuAppCard[] = [
    ...
    },
    {
      ...
      category: 'prioritas',
      badge: currentUser.role === 'admin' || currentUser.role === 'operator' ? 'Konfidensial' : 'Terkunci Admin',
      icon: AlertTriangle,
      iconBg: currentUser.role === 'admin' || currentUser.role === 'operator' ? 'bg-rose-600' : 'bg-slate-500',
    },
    {
      id: 'sp_damai',
      ...
      category: 'prioritas',
      badge: currentUser.role === 'admin' || currentUser.role === 'operator' ? 'Restorative' : 'Terkunci Admin',
      icon: Handshake,
      iconBg: currentUser.role === 'admin' || currentUser.role === 'operator' ? 'bg-cyan-600' : 'bg-slate-500',
    },
    {
      id: 'buku_tamu',
