...
export const InfografisWelcomeModal: React.FC<InfografisWelcomeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-3 bg-slate-950/75 backdrop-blur-sm overflow-hidden animate-in fade-in duration-150">
      {/* MODAL MAIN CONTAINER */}
      <div
        className={`bg-white w-full flex flex-col overflow-hidden shadow-2xl transition-all duration-200 border border-slate-200 ${
          isFullscreen
            ? 'fixed inset-0 rounded-none h-screen max-w-none'
            : 'max-w-6xl h-full sm:h-[95vh] sm:rounded-3xl'
        }`}
      >
        {/* 1. TOP HEADER BAR (MATCHES SCREENSHOT) */}
        <div className="px-5 sm:px-6 py-3 border-b border-slate-200 bg-white flex items-center justify-between flex-shrink-0">
          {/* Left: Green dot + UPTD SMP NEGERI 7 PASURUAN • Dokumen Resmi Alur & Layanan Terpadu */}
          ...
          {/* Right: [ZoomOut, 100%, ZoomIn], [Maximize], [X] */}
          <div className="flex items-center gap-2.5">
            {/* Zoom Controls Pill */}
        ...
        {/* 2. MAIN BODY CANVAS WITH LIGHT MINT GRADIENT */}
        <div className="flex-1 overflow-y-auto bg-gradient-to-b from-[#dff2ea] via-[#edf7f2] to-[#f8fbf9] p-4 sm:p-8 flex flex-col items-center">
          {/* Circular PASS TEMENAN Logo with Radiant Aura */}
          ...
          {/* Pill Badge: APLIKASI SAHABAT SPANJU */}
          ...
          <div className="w-full max-w-3xl bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden transition-transform">
            <div
              style={{
        ...
        {/* 3. BOTTOM FOOTER BAR (MATCHES SCREENSHOT) */}
        <div className="px-5 sm:px-6 py-3.5 border-t border-slate-200 bg-white flex flex-col sm:flex-row items-center justify-between gap-3 flex-shrink-0">
          ...
          {/* Right: Masuk ke Login Aplikasi > Button */}
          <button
            type="button"
            onClick={onProceedToLogin ? onProceedToLogin : onClose}
